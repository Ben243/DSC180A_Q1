...
def train_model(data_path, pickle_path):
    ...
    loss_forest.fit(loss_X_train, loss_y_train)
    ...
    # latency_X_norm = (X_pca - X_pca.mean()) / X_pca.std()
    latency_X_pca = PCA(n_components=PCA_COMPONENTS).fit_transform(latency_X)
    ...
    latency_forest.fit(latency_X_train, latency_y_train)
